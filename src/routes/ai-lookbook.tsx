...
import { ReplicatePredictionStatus } from '@/constants/replicate';
  ...
  const handleGenerate = async () => {
    ...
    try {
        ...
        const pollStage1 = async () => {
            ...
            if (statusData.status === ReplicatePredictionStatus.SUCCEEDED) {
                stage1Complete = true;
                toast.success('Virtual try-on complete! Applying face swap...');
                return true;
            } else if (statusData.status === ReplicatePredictionStatus.FAILED 
                || statusData.status === ReplicatePredictionStatus.CANCELED) {
        ...
        const pollStage2 = async () => {
            ...
            if (childResponse.child.status === ReplicatePredictionStatus.SUCCEEDED 
                && childResponse.child.output) {
                return { complete: true, output: childResponse.child.output };
            } else if (childResponse.child.status === ReplicatePredictionStatus.FAILED 
                || childResponse.child.status === ReplicatePredictionStatus.CANCELED) {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-[1400px]">
      <div className="text-center mb-12">
        ...
        {/* Accessory Notice */}
        <div className="mt-6 mx-auto max-w-2xl bg-amber-50 border border-amber-200 rounded-lg p-4 text-left">
